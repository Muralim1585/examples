...
library(rattle)           #print the business rules for bagging and boosting model
library(inTrees)          #to extract the business rules from random forest model
setwd("/Users/Rahul/Documents/Rahul Office/IIMB - BAI4/Classes @ IIMB/Company Fraud")
```
...
```{r, echo=TRUE}
raw.data <- read.csv("/Users/Rahul/Documents/Rahul Office/IIMB - BAI4/Classes @ IIMB/Company Fraud/IMB 575 FRAUD ANALYTICS.csv",head=TRUE,na.strings=c("", " ", "NA"), sep=",")
...
```{r}
trainIndex <- createDataPartition(filter.data$Manipulater, p = 0.80, list=FALSE)
...
####Train and test dataset with needed variabbles
```{r}
...
correlationMatrix <- cor(model.data[,c(1:8)])
...
rf.bootstrap.model <- train(model.train[,1:8], model.train[,9], 
                  method='rf', 
...
rf.up.model <- train(model.train[,1:8], model.train[,9], 
                  method='rf', 
                  trControl=objControl,  
                  metric = "ROC",
                  prox=TRUE,allowParallel=TRUE)
...
rf.up.rules <- extractRules(rf.up.treelist,model.train[,c(1:8)], ntree = 500)
rf.up.rules.metric <- getRuleMetric(rf.up.rules,model.train[,c(1:8)],model.train[,9])
rf.up.rules.metric <- pruneRule(rf.up.rules.metric,model.train[,c(1:8)],model.train[,9])
rf.up.rules.metric <- selectRuleRRF(rf.up.rules.metric,model.train[,c(1:8)],model.train[,9])

#readable rules
print(presentRules(rf.up.rules.metric, colnames(model.train[,c(1:8)])))
...
rf.down1.model <- train(model.train[,1:8], model.train[,9], 
                  method='rf', 
                  trControl=objControl,  
                  metric = "ROC",
                  prox=TRUE,allowParallel=TRUE)
...
ada.model <- train(model.train[,1:8], model.train[,9], 
                  method='AdaBoost.M1', 
                  trControl=objControl,
...
ada.up.model <- train(model.train[,1:8], model.train[,9], 
                  method='AdaBoost.M1', 
                  trControl=objControl,
                  tuneGrid = search.grid,
                  metric = "ROC",
...
ada.down.model <- train(model.train[,1:8], model.train[,9], 
                  method='AdaBoost.M1', 
                  trControl=objControl,
                  tuneGrid = search.grid,
                  metric = "ROC",
...
xg.model <- train(model.train[,1:8], model.train[,9], 
                  method='xgbTree', 
                  trControl=objControl,
                  tuneGrid = search.grid,
                  metric = "ROC",
...
```{r, echo=TRUE}
caretPredictedClass <- predict(xg.model, model.test[1:8], type = "raw")
confusionMatrix(caretPredictedClass,model.test$Manipulater)
```

ROC plot for xgboost on test set
```{r, echo=TRUE}
xg.pred <- predict(xg.model, model.test[1:8], type = "prob")[,2]
...
xg.up.model <- train(model.train[,1:8], model.train[,9], 
                  method='xgbTree', 
                  trControl=objControl,
                  tuneGrid = search.grid,
                  metric = "ROC",
...
Confusion Matrix for xgboost on test set
```{r, echo=TRUE}
caretPredictedClass <- predict(xg.up.model, model.test[1:8], type = "raw")
confusionMatrix(caretPredictedClass,model.test$Manipulater)
```

ROC plot for xgboost on test set
```{r, echo=TRUE}
xg.pred <- predict(xg.up.model, model.test[1:8], type = "prob")[,2]
xg.prediction <- prediction(xg.pred,model.test$Manipulater)
xg.perf <- performance(xg.prediction, "tpr","fpr")
...
xg.down.model <- train(model.train[,1:8], model.train[,9], 
                  method='xgbTree', 
                  trControl=objControl,
                  tuneGrid = search.grid,
                  metric = "ROC",
...
Confusion Matrix for xgboost on test set
```{r, echo=TRUE}
caretPredictedClass <- predict(xg.down.model, model.test[1:8], type = "raw")
confusionMatrix(caretPredictedClass,model.test$Manipulater)
```

ROC plot for xgboost on test set
```{r, echo=TRUE}
xg.pred <- predict(xg.down.model, model.test[1:8], type = "prob")[,2]
xg.prediction <- prediction(xg.pred,model.test$Manipulater)
xg.perf <- performance(xg.prediction, "tpr","fpr")
...
nn.objModel <- train(model.train[,1:8], model.train[,9], 
                  method='nnet', 
                  trControl=objControl,  
                  metric = "ROC",
                  maxit = 1000,
